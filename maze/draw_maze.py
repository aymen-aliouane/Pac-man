...
def build_maze_tiles(
    ...
    for y in range(rows):
        ...
        for x in range(cols):
            ...
            tiles[y].append(
                build_tiles(cell, settings, right, top, left, bottom)
            )
...
def build_tiles(
    ...
    Parameters:
        ...
        right, top, left, bottom: int = the neighbor
    ...
    # Helpers to know if the cell has a wall for each direction.
    def has_left(value: int) -> int:
        return value & 8

    def has_bottom(value: int) -> int:
        return value & 4

    def has_right(value: int) -> int:
        return value & 2

    def has_top(value: int) -> int:
        return value & 1
    ...
    if (
        has_right(cell)
        and (not has_top(cell))
        and (not has_right(top))
        and (not has_top(right))
    ):
        pygame.draw.line(surface, color,
                         (inner_edge, 0), (edge, 0), 2)

    if (
        has_left(cell)
        and (not has_top(cell))
        and (not has_left(top))
        and (not has_top(left))
    ):
        pygame.draw.line(surface, color,
                         (0, 0), (offset, 0), 2)
    ...
    if (
        has_right(cell)
        and (not has_bottom(cell))
        and (not has_right(bottom))
        and (not has_bottom(right))
    ):
        pygame.draw.line(surface, color,
                         (inner_edge, edge), (edge, edge), 2)

    if (
        has_left(cell)
        and (not has_bottom(cell))
        and (not has_left(bottom))
        and (not has_bottom(left))
    ):
        pygame.draw.line(surface, color,
                         (0, edge), (offset, edge), 2)

    if (
        has_top(cell)
        and (not has_right(cell))
        and (not has_top(right))
        and (not has_right(top))
    ):
        pygame.draw.line(surface, color,
                         (edge, 0), (edge, offset), 2)
    ...
    if (
        ...
        pygame.draw.line(surface, color,
                         (edge, inner_edge), (edge, edge), 2)

    if (
        has_top(cell)
        and (not has_left(cell))
        and (not has_top(left))
        and (not has_left(top))
    ):
        pygame.draw.line(surface, color,
    ...
    if (
        ...
        pygame.draw.line(surface, color,
                         (0, inner_edge), (0, edge), 2)

    # handle corners of cells like '┐' or 'L'
    if (
        (not has_right(cell))
        and (not has_bottom(cell))
        and has_bottom(right)
        and has_right(bottom)
    ):
        pygame.draw.line(surface, color,
                         (inner_edge, inner_edge), (inner_edge, edge), 2)
        pygame.draw.line(surface, color,
                         (inner_edge, inner_edge), (edge, inner_edge), 2)
    ...
    if (
        ...
        pygame.draw.line(surface, color,
                         (inner_edge, 0), (inner_edge, offset), 2)
        pygame.draw.line(surface, color,
                         (inner_edge, offset), (edge, offset), 2)
    ...
    if (
        ...
        pygame.draw.line(surface, color,
                         (offset, inner_edge), (offset, edge), 2)
        pygame.draw.line(surface, color,
                         (0, inner_edge), (offset, inner_edge), 2)
